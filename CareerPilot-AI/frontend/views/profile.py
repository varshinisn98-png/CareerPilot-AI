...
def show():
    ...
    page_header("Profile", "Manage your account and settings", "👤")
        ...
        st.markdown(
            ...
            <div style="background:linear-gradient(135deg,rgba(79,139,249,0.15),rgba(124,58,237,0.15));
                ...
                <div style="width:80px;height:80px;background:linear-gradient(135deg,#4f8bf9,#7c3aed);
                    ...
                    margin:0 auto 1rem auto;font-size:2.5rem;color:white;font-weight:700;">
                    {user.get('full_name','U')[0].upper()}
                </div>
                <h2 style="color:#f1f5f9;font-size:1.2rem;font-weight:700;margin:0 0 4px 0;">
                    {user.get('full_name','')}</h2>
                <p style="color:#64748b;font-size:0.85rem;margin:0 0 12px 0;">{user.get('email','')}</p>
                <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);
                    border-radius:20px;padding:3px 12px;display:inline-block;">
                    <span style="color:#22c55e;font-size:0.78rem;font-weight:600;">✅ Active</span>
                </div>
            </div>
            """,
    ...
    with col_r:
        ...
        if save:
            payload = {}
            if new_name != user.get("full_name"): payload["full_name"] = new_name
            if new_email != user.get("email"): payload["email"] = new_email
            if payload:
                try:
                    res = httpx.put(f"{API_BASE}/user/me", headers=_headers(), json=payload, timeout=10)
        ...
        st.markdown('<p style="color:#64748b;font-size:0.85rem;">Deleting your account is permanent.</p>',
                    unsafe_allow_html=True)
        ...
        if st.session_state.get("confirm_delete"):
            ...
            with c1:
                if st.button("Yes, Delete Everything", type="primary", use_container_width=True):
                    try: httpx.delete(f"{API_BASE}/user/me", headers=_headers(), timeout=10)
