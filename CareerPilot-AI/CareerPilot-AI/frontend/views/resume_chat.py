                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if _is_gemini_error(detail):
                            st.error("🔑 **Gemini API key not configured.** Add your key to `backend/.env` → `GEMINI_API_KEY=your-key` then restart the backend. Get a free key at https://aistudio.google.com/app/apikey")
                        else:
                            st.error(detail or "Sorry, couldn't process that. Try again.")
    ...
    if st.session_state["chat_messages"]:
        ...
        with col1:
            if st.button("🗑️ Clear Chat", use_container_width=True):
                try:
                    httpx.delete(f"{API_BASE}/chat/history/{resume_id}", headers=_headers(), timeout=10)
                except Exception:
                    pass
                st.session_state["chat_messages"] = []
